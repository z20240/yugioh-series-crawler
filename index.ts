...
async function getPropertiesFromCardRushPage(id: number, seriesIdx: string) {
  ...
  const doRequest = async (id: number) => {
    ...
    while (queryTimes < 1) {
      try {
        ...
        console.log(
          "🚀 ~ file: index.ts:27 ~ doRequest ~ [Request Failed]: id=",
          id,
          "url: ",
          `${CardRushUrl}${id}`,
          " retry..."
  ...
  try {
    ...
    return {
      series: series.trim(),
      imageUrl: imageUrl,
      plantRarity: RarityToIdMap[Rarity[plantRarity]] ?? "0",
      jpName: jpName.trim(),
    };
  } catch (e) {
...
// http://220.134.173.17/gameking/card/ocg_list.asp?call_item=99&call_data=&call_sql=Select%20*%20from%20ocg&Page=2450
// total 2458 頁
async function getPropertiesFromChiaoChiaoWuPage(
  series: string,
  option?: { id?: number | string; imageUrl?: string }
): Promise<{ chName: string; uid: string }> {
  try {
    const { data: page } = await axios.get(
      `${Constants.OCG_CHIAO_CHIAO_WU}${series}`,
      {
    ...
    console.log(
      "🚀 ~ file: index.ts:53 ~ getPropertiesFromChiaoChiaoWuPage ~ chName:",
      chName,
      " uid:",
      uid
    );
    ...
    console.log(
      "🚀 ~ file: index.ts:72 ~ getPropertiesFromChiaoChiaoWuPage ~ series=",
      series,
      option?.id,
      option?.imageUrl
    );
...
async function fetchNecessaryData(
  ...
  {
    seriesIdx,
  }: { seriesIdx: string; series: string; period: number; chTitle: string }
): Promise<CSV_Raws> {
  ...
  const {
    ...
    plantRarity,
    jpName,
  } = await getPropertiesFromCardRushPage(id, seriesIdx);

  if (!seriesNo && !imageUrl) return { csv1: [], csv2: [] };

  const pathName = isValidSeriesNumber(seriesNo)
    ? `./data/${seriesIdx}/${seriesNo}_${plantRarity}.jpg`
    : `./data/${seriesIdx}/${jpName}.jpg`;

  if (imageUrl) await downloadImage(imageUrl, pathName);

  if (!seriesNo || !isValidSeriesNumber(seriesNo))
    return { csv1: [], csv2: [seriesIdx, "", jpName, "", "", pathName] };
...
async function main() {
  await Promise.all(
    Object.entries(data).map(
      async ([key, { series, period, range, chTitle }]) => {
        ...
        const csvlist = await Promise.all(
          serieslist.map(async (id) => {
            await BPromise.delay(
              randomDelay[~~(Math.random() * randomDelay.length)]
            );
            return fetchNecessaryData(id, {
              seriesIdx: key,
              series,
              period,
              chTitle,
        ...
        console.log(
          "🚀 ~ file: index.ts:114 ~ Download images finished, starting to create CSV."
        );

        const { csv2 } = csvlist.reduce(
          (acc: CSVs, cur: CSV_Raws) => {
            if (!cur) return acc;
            return {
        ...
        writeToCsv(
          [seriesHeader, [1, period, "", chTitle, series, key, ""]],
          `./系列.csv`,
          { append: true }
        );
